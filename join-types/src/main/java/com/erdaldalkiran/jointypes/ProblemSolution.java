...
public class ProblemSolution implements IStreamer {
    ...
    public void run() {
        ...
        KTable<Long, XDockDeliveryCount> xDockDeliveryCountTable = deliveryStream
            .selectKey((key, value) -> value.getXDockId())
            .groupByKey()
            .count()
            .join(
                xDockTable,
                (count, xDock) -> XDockDeliveryCount.builder()
                    .id(xDock.getId())
                    .count(count)
                    .xDockName(xDock.getName().toString())
                    .build()
            );
        xDockDeliveryCountTable.toStream().print(Printed.toSysOut());
